...
public class TicketDatabaseRepository implements TicketRepository {
    ...
    @Override
    public List<Ticket> ticketsByEventId(final EventId anEventId) {
        Objects.requireNonNull(anEventId, "id cannot be null");
        return this.ticketJpaRepository.findByEventId(UUID.fromString(anEventId.value())).stream()
                .map(TicketEntity::toTicket)
                .toList();
    }
